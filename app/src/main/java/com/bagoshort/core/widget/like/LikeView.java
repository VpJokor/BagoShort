package com.bagoshort.core.widget.like;
...
/**
 * 使用方法
 *         <com.xiaopoquan.likeview.btn.LikeView
 *             android:id="@+id/like"
...
public class LikeView extends View implements Checkable {
    ...
    private  float mInnerShapeScale;
    ...
    private static final float RING_WIDTH_RATIO = 1.2f;
    private static final float SIZE_RATIO = 2.4f;
    ...
    public LikeView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mInnerShapeScale = array.getFloat(R.styleable.LikeView_innerShapeScale, RADIUS_INNER_SHAPE_SCALE);
    ...
    private void drawDotWithRing(Canvas canvas, int radius, int color) {
        ...
        float  ringPercent = (Math.min(1f - mCurrentPercent, 1f)) * 0.2f;
        //环形宽度缩小
...
//                //逐渐放大
//                float percent = calcPercent(100f, 220f, animatedValue);//此阶段未达到最大半径
//                mCurrentRadius = (int) (1.0 * mRadius * percent);
//                if (animatorArgb != null && animatorArgb.isRunning()) {
//                    mCurrentColor = (int) animatorArgb.getAnimatedValue();
//                }
...
//                invalidate();
//            }
//            else if (animatedValue <= 220) {
//                float percent = calcPercent(100f, 220f, animatedValue);//半径接上一阶段增加，此阶段外环半径已经最大值
//                mCurrentPercent = Math.min(1.2f - percent, 1f);//用于计算圆环宽度，最小0.2，与动画进度负相关
//                mCurrentRadius = (int) (1.0 * mRadius * percent);
//                if (animatorArgb != null && animatorArgb.isRunning()) {
//                    mCurrentColor = (int) animatorArgb.getAnimatedValue();
//                }
...
//                invalidate();
//            }
            else if (animatedValue <= 300) {
                //内环半径增大直至消亡
                float percent = calcPercent(220f, 300f, animatedValue);
                mCurrentPercent = percent;
                mCurrentRadius = (int) (1.2 * mRadius);//外环半径不再改变
                mCurrentState =  State.RING_DOT__HEART_VIEW;
                invalidate();
            }
            else if (animatedValue <= 600) {
                float percent = calcPercent(300f, 600f, animatedValue);
                mCurrentPercent = percent;
                mCurrentState =  State.DOT__HEART_VIEW;
                invalidate();
                if (animatedValue == 600) {
                    animatorTime.cancel();
                    if (!isChecked) {
                        restoreDefaultView();
                    }else {
...
//        Log.i("onDetachedFromWindow","onDetachedFromWindow");
     ...
     */
    public void setInnerShapeScale(float innerShapeScale) {
